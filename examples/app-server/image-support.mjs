 ...
 *
 * Usage:
 *   node examples/app-server/image-support.mjs                    # Uses bundled bull.webp
 *   node examples/app-server/image-support.mjs /path/to/image.png # Uses custom image
 */
...
const appServer = createCodexAppServer({
  defaultSettings: { minCodexVersion: '0.155.0', idleTimeoutMs: 30000 },
});
...
try {
  ...
  async function main() {
    ...
    if (!process.argv[2]) {
      ...
      console.log('  node examples/app-server/image-support.mjs /path/to/image.png\n');
    } else {
